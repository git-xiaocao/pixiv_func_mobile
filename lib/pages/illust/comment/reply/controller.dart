...
class IllustCommentReplyController extends GetxController {
  final int id;

  IllustCommentReplyController(this.id) : source = IllustCommentReplyListSource(id);

  final IllustCommentReplyListSource source;
  ...
  String get commentInputLabel => isReplies ? I18n.replyComment.trArgs([_repliesComment!.user.name]) : I18n.replyComment.trArgs(['']);

  void onCommentAdd({String? text, int? stampId}) {
    Get.find<ApiClient>().postCommentAdd(id, comment: text, parentCommentId: repliesComment?.id ?? id, stampId: stampId).then((result) {
